...
**The review's own sections are now all closed**, and *Open — found after the
review* below is where anything found since goes. The entries there were found
by running the platform rather than by reading it — F24 and F25 within a day of
each other, and F26 by a required CI job going red on a defect in the harness
itself rather than in the platform.
...
| F5 | ~~Unbounded all-namespace reads~~ **closed.** API-server paging via `--chunk-size`, retained items capped, truncation recorded as an evidence gap — and the spike is gone too. A list read is now decoded as it arrives (`app/kubernetes/json_stream.py`): items past `MAX_LIST_ITEMS` are counted and dropped, so nothing ever holds the whole document. Measured through the real executor with `python scripts/payload_bench.py --parse-scan`: peak was **5.9 MB at 2,000 pods, 29.7 MB at 10,000, 74.3 MB at 25,000** and is now **flat at 8.4 MB from 5,000 pods upward** — 8.8x at the top of the range. The trade is a constant ~40% more at or below the cap (8.3 MB against 5.9 MB at 2,000 pods), which is inherent to decoding elements one at a time rather than a document whole: 8.51 MB against 6.13 MB measured directly, because one `json.loads` shares more strings than 2,000 separate decodes. The unbounded number was the one worth removing. **What made it safe to do was a fuzz against `json.loads`**, one byte per read: it caught `raw_decode` accepting `3` out of `3.` and `1234` out of `12345678901234`, which would have filed a different number as evidence. Every array element ends at `,` or `]`, and that delimiter is what the reader waits for. | 3d done, 2d done |
...
| # | Finding | Effort |
|---|---|---|
| F31 | ~~The console offered a fix for a workload the diagnosis was not about~~ **fixed.** The investigation page mounted `RemediationPanel`, which composed its own YAML patch and "Apply Fix" commands in the browser, targeting `firstAffectedWorkload` — the **first problematic workload in the namespace**, whatever the diagnosis said. Reproduced against `docs/qa/audit-faults.yaml` plus a default-deny NetworkPolicy: the diagnosis selected `network.service_without_endpoints` and the backend plan targeted **Service `payments/checkout-svc`**; on the same page the panel showed a Deployment patch for **`payments/archiver`** — the PVC-blocked pod, unrelated — and "Apply Fix" put `kubectl edit deployment archiver -n payments # CHANGES STATE` on the clipboard. An operator responding to a broken Service selector was invited to edit and roll out a different production workload, from the product whose premise is that nothing is asserted without evidence. **Three more defects in the same panel**: every risk level except `Medium` rendered as `good`, so the two `High` plans the platform emits (marking a StorageClass default; opening ingress through a default-deny NetworkPolicy) showed green; step 2 was a hardcoded "Restart or roll out the affected deployment", which `_rollable()` refuses for bare pods; and "Apply Fix" applied nothing while "Create GitHub PR" downloaded a Markdown file. **The correct component already existed**: `RemediationPlanPanel` renders the plan `app/remediation/` keys on the selected hypothesis — its target, caveats, required RBAC checks and the backend's patches labelled "never run by this platform" — and was unmounted in `38f895b` when the page became a report document, which kept the legacy panel. **Why nothing caught it is the useful part**: the builders had fifteen tests and eight mutation pairs, every one confirming correct YAML *for the workload they were handed*; none asked whether that was the workload the diagnosis named. Found by driving every console route and action in a real browser rather than loading pages. Fixed by mounting `RemediationPlanPanel` and deleting the legacy panel and builders (verified unreferenced by grep and by `tsc -b`); an unrecognised risk level now renders neutral instead of borrowing `Low`'s green. Verified in Chrome against the faulted cluster: target `Service/payments/checkout-svc`, amber for Medium, no mention of `archiver` as a fix, no Apply Fix control, no console errors. `routes.test.tsx` asserts the page never offers the unrelated workload — mutation-verified to fail on `name: archiver` itself, after a first version failed on a setup line and so never ran the assertion that names the defect. | done |
| F30 | ~~Every report download in the console 401'd~~ **fixed.** The same root cause as F29, found by asking what *else* a browser sends without a header. `ArtifactsPanel` and `ReportsPage` linked all three rendered formats with `<a href>` straight at the API, and a plain navigation cannot carry an `Authorization` header any more than an `EventSource` can — so with authentication configured, clicking "PDF Report" was answered **401** and the browser saved the JSON error body under the name of a report, which opens as a broken PDF and explains nothing. Measured against a token deployment: pdf, json and markdown **all 401 as a browser sends them, all 200 with the header**. The PDF is a headline feature with its own document (`docs/INCIDENT_REPORTS.md`) and a hand-rolled emitter, and it could not be downloaded from the product's own console. **Fixed by fetching with the credential and saving the blob**, through the object-URL idiom the console already used for remediation artefacts — `saveBlob` is now the one place a file reaches the user, with `downloadText` delegating to it, because two implementations of that dance are two places to forget `revokeObjectURL`. A failed download now says so in the UI rather than silently saving nothing, since silently-saving-the-wrong-thing is precisely what the defect looked like from outside. **And the fix was half inert until a live browser said so**: the filename is read from the server's `Content-Disposition` so the platform stays the one thing that names a report, but CORS exposes only a handful of response headers to script, so cross-origin that read returned `null` — no error anywhere, just a silent fall back to a generic name, which reads exactly like a header the server never sent. `expose_headers` now names `Content-Disposition` and `X-Correlation-ID`, the latter being the id operators are invited to quote and could not see either. Verified in Chrome: PDF 47,244 bytes beginning `%PDF`, JSON 459,682, Markdown 17,662, each under `investigation-<id>.<ext>`. Guarded by tests asserting the request carries the credential, that a refusal raises instead of saving an error body under a report's name, and — asserted on the response the middleware actually produces, since `expose_headers` correct in configuration and never passed to the middleware reads identically to a working one — that both headers are exposed. Each mutation-verified. | done |
| F29 | ~~The console's live progress stream 401'd in every authenticated deployment~~ **fixed.** `EventSource` cannot send an `Authorization` header — the API has no option for it — and `/investigations/{id}/events` sits behind `require_principal` like every other endpoint, so the console's stream request arrived anonymous and the platform correctly answered **401**. Since `AUTH_MODE` lost its default that is *every* deployment, so the console has never streamed anywhere real: it opened the stream, was refused, and fell back to polling for the whole of every investigation. **This is the second independent fault on this one path**, and the fallback hid both — the first was the browser's named-event dispatch rule (`99b9d27`), and fixing it could only have been verified with authentication switched off, because underneath it the request was still being refused. Measured in Chrome against a token deployment: **zero events of every type and an error**, the identical signature to the defect already fixed. **Fixed by reading the stream with `fetch`** (`frontend/src/services/eventStream.ts`), which can carry the credential the rest of the console already sends; after, **queued 1, started 1, progress 25, completed 1** over 28 monotonic frames, and the console's own request logs 200 where the two "before" measurements are the only 401s in the log. A token in the query string was rejected as the alternative — `API_TOKENS` are long-lived and a URL lands in access logs, proxy logs and browser history — and a short-lived stream ticket would be a second credential type to mint, store in both backends and expire, for a problem a header already solves. The cost is parsing SSE ourselves (a frame ends at a blank line, and chunk boundaries are not frame boundaries) and 0.5 KB gzipped. **What could not see it**: the hook's test double called `onmessage` directly, so no request was ever made and no header could be missing — the same "models a wire the server does not produce" gap that hid the first fault, now replaced by a double that writes the real bytes into a `ReadableStream`; and the required CI check streams with an `Authorization` header, **which is a header no browser can send**, so curl proving the server streams says nothing about whether the client can reach it. `docs/INVESTIGATION_API.md` recommended `EventSource` throughout, which is the advice that walks a consumer straight into the 401. Guarded by tests asserting the request carries the credential, that no `EventSource` is constructed, and that a refused stream falls back to polling; each mutation-verified. | done |
| F28 | ~~Progress reporting blocked the event loop~~ **fixed.** On the distributed store a progress event is a committed Postgres row plus a Redis publish, and every call site — the collection scheduler, the playbook orchestrator, the investigation runner — is a coroutine on the event loop. `ProgressReporter.report` was synchronous, so each of the ~50 events an investigation emits stopped the worker advancing **any** task: not the HTTP request beside it, not an SSE frame, not a message on any attached agent's gRPC stream. `create`, `enqueue` and `mark_running` did the same on the submit and start paths. Measured against real Postgres and Redis with `scripts/loop_bench.py`: **2,592 of 2,688 job-store calls made on the loop, the loop blocked 97% of wall clock, p50 lag 75 ms and p99 306 ms** — on a platform whose entire measured work per investigation is 0.21 s. **This is the answer to the question `docs/PERFORMANCE_ENVELOPE.md` had carried open for nine milestones**, and the earlier stack sampling had already photographed it without being able to read it: a sampler cannot tell a thread *waiting for work* from a thread *stuck in a socket*, so "92% idle, every non-idle sample in a Postgres or Redis socket wait" and "blocked 97% of wall clock" are the same worker described twice. **Fixed by making `ProgressReporter` awaitable**, which puts the thread dispatch in the one implementation that does I/O rather than at each call site — a caller only ever awaits, so there is no second thing to remember at a new one. Ordering is unchanged because the caller awaits: the row is committed before the collector's coroutine proceeds, so sequences stay monotonic and a terminal event cannot overtake the progress events before it. After: **0 of 2,688 on the loop, blocked 33–36%, p50 lag 0.8 ms, and 48 investigations in 5.06 s against 10.04 s**, bracketed after/before/after because a laptop's throughput drifts. **The suite passed with the defect present and would have passed with the fix reverted**, because no test had ever driven a `ProgressReporter` — every progress test called `store.publish()` directly, so the bridge from *a collector finished* to *an event reached the stream* was covered at neither end. `tests/test_progress_reporting.py` closes that and asserts the loop keeps advancing, observed by a heartbeat task rather than by inspecting the calling thread — a thread check would have to name each store method, which is the second table this codebase keeps refusing to write. Registered in `scripts/mutation_check.py`. **What it does not change is the conclusion**: throughput is still flat against `JOB_MAX_CONCURRENT`, so add workers, not slots — the ceiling is roughly 2x higher, not differently shaped. | done |
| F27 | ~~An agent-collected evidence record did not say whose RBAC produced it~~ **fixed.** `equivalent_command` is the evidence spine's answer to how a fact was obtained — every record carries the invocation that would produce the same bytes. The kubeconfig path records `--as <caller>`, because per-request impersonation is how F13's "the platform cannot see more than you can" is delivered; the agent path applies the same identity as `Impersonate-User` headers and recorded **no identity at all**. No data was wrong, which is why it lasted: the reads *were* impersonated on both paths. What was wrong is the record, and it costs three things — the two transports disagreed about whose RBAC produced the same read, so investigations were not comparable on the one field a differential harness uses to find behavioural divergence; a human running the recorded command read as themselves, which on a cluster-admin kubeconfig returns **more** than the investigation saw; and an audit of an agent-served investigation could not answer who a fact was collected for. **Fixed in the agent and gated on what happened rather than on what was asked.** `Collector.impersonating()` is now one condition asked by both the headers and the record, because two copies drift and the drift is silent in both directions. It is deliberately *not* rendered on the platform side, which knows the actor it sent but cannot know whether the agent applied it — an agent enrolled before impersonation shipped discards the actor, and a command claiming `--as` there would be the same false record reversed. That gate is also what preserves the true positive: a non-impersonating agent beside an impersonating kubeconfig genuinely reads as two different identities. Found by `scripts/provider_diff.py` over a live 57-record namespace with status and content both clean — **33 command differences of exactly one shape**. After the fix, **0 differences across all three nets**, and still 33 with the agent not impersonating. Four Go mutations watched fail, including rendering the identity unconditionally. **A second finding came out of the same investigation**: `soak_bench.py` never passed `--impersonate`, so with `--agent` — where 100% of collection goes through the agent — its `grant_caller_rbac` guard was protecting a path the headline run does not take, and no soak had ever exercised F13 on the agent path. Proved by mutating the grant away: the pre-fix agent publishes 40/40 usable at exit 0; the impersonating one gives 0/40, REFUSED, exit 1. | done |
| F26 | ~~The differential suite could not tell a provider divergence from a cluster that moved~~ **fixed.** `TestEveryCollectorAgrees` runs the baseline collector graph twice against one cluster, once through an agent and once through a kubeconfig, and compares the values each derived — which is exactly what makes it worth running, and exactly why two reads seconds apart see two different clusters. It **failed the required `integration-verify` job on `da5de44`** with `k8s.deployments.unhealthy_deployments differs`, `unavailable_replicas` 1 against 0, while all 48 of its own deployment checks passed beside it: the platform's Deployment was replacing a pod between the two reads. A named volatile-field list could never have covered this — `unhealthy_deployments` is a *finding*, not a clock, and what was wrong with it was that it happened to be moving right then. This is the failure mode a harness dies of rather than the one it catches: a required job that cries wolf is skipped exactly like a flaky one, and the same lesson is already recorded one test below, where a metrics equality failed CI on a node reading exactly 300m. **Fixed by bracketing each comparison** — agent, kubeconfig, agent again — so a value that moved between the two reads of one provider is churn rather than disagreement. One bracket catches every one-time change: with samples at t1 < t2 < t3 and a single change at T, either T is inside [t1, t3] and the bracketing reads disagree, or all three agree; only a value that changes and changes back inside the window survives, which is the residue `scripts/provider_diff.py` already handles by asking whether a difference swaps sides across two runs. **The half that would have shipped unnoticed is the opposite one.** An exclusion with no floor passes hardest exactly when the cluster is least readable, so the exclusion is per *value* rather than per section (a moving `phase` on one pod does not excuse a divergence on the pod beside it), `Comparison.refusal()` refuses a comparison the cluster churned away instead of calling it agreement, and the baseline test now requires both paths to have collected something usable — every other assertion in that class is satisfied by two providers that failed identically, which is what a dead Docker daemon produces. The discriminator is `tests/differential.py`, tested hermetically by `tests/test_differential_control.py` because the suite that uses it skips wherever no cluster is present; both directions are in `scripts/mutation_check.py`. **Verified by reproducing the trigger**, against a deployment flapping its replica count: without the fix the CI failure reproduces exactly, same test and same kind; with it, 12/12 pass; and with F25 reverted on the agent path to inject a genuine divergence *while the churn continues*, 5 tests fail — so the exclusion did not disarm the suite. The refusal guard did not fire at 4↔30 replicas flapping and rests on its unit tests. | done |
| F25 | ~~`MAX_LIST_ITEMS` did not apply on the agent path~~ **fixed.** The cap lived inside `KubectlExecutor`, so it bounded the kubeconfig path and nothing else, and `RemoteAgentProvider._truncations` was initialised and **never appended to** — it existed only to satisfy the `ClusterProvider` protocol. An agent-reached cluster was therefore read with **no ceiling at all**, and `collection_limits.truncated` reported `false` for a read that had never been bounded. That matters twice over: `docs/PERFORMANCE_ENVELOPE.md` sizes the platform's per-worker memory on that ceiling holding, and an agent is the transport real fleets use, so the published envelope did not apply where it mattered most; and a truncation is a *citable evidence gap*, so a silent cap makes an investigation that saw part of a cluster read as a complete view of a small one. Measured live with `MAX_LIST_ITEMS=3` against a ten-pod namespace: the kubeconfig path reported `total_pods: 3`, `truncated: true` and four truncation records naming returned and retained; the agent path reported `total_pods: 10`, `truncated: false` and none — the same cluster, read two ways, disagreeing about its size. **Fixed by extracting the rule to `app/kubernetes/list_limit.cap_items`, called by both providers**, because two implementations of one rule drift; it lives under `app/kubernetes/` rather than `app/providers/` only because `app/providers/__init__` imports `local_kubectl`, which imports the executor, making the other direction a circular import. **The first version of the fix introduced a divergence in the opposite direction and the live run caught it**: gated on the payload merely carrying an `items` key, it also truncated `kubectl top` — text on the kubeconfig path and therefore never capped there, but a metrics.k8s.io list through an agent — and the verification run came back with five truncation records against the kubeconfig path's four. It is now gated on `request.is_list`, the counterpart of the executor's `_is_list_read`, so both providers bound exactly the same set of reads; the repeat run gives four identical records and, at the shipped default of 2,000, neither path caps anything or invents a truncation. Nine tests, four mutations watched fail, two added to `scripts/mutation_check.py`. Found by diffing the recorded `equivalent_command` of an agent-served investigation against a kubeconfig-served one after the *status* diff came back clean across four scopes — the kubeconfig reads carried `--chunk-size=500` and the agent's carried no limit at all, which is what raised the question. | done |
| F24 | ~~A pod with more than one container has no logs at all on the agent path~~ **fixed.** `PodLogsCollector` and `PodPreviousLogsCollector` both send `all_containers`, which kubectl expands *client-side* — it reads the pod, fetches each container's log and concatenates. The agent had no such expansion: `resolveLogs` read `container`, `tail` and `previous` and nothing else, so it issued one read naming no container and the API server answered a multi-container pod with `BadRequest: a container name must be specified for pod X, choose one of: [...]`. The whole log read failed. **Sidecars are the common case** — a service mesh, a log shipper, a secrets agent — so an agent-reached cluster lost the single most useful evidence a crash has, while the same cluster read through a kubeconfig kept it, and it was silent in the usual way: a failed evidence record and an investigation that succeeds with a gap. **Fixed by performing the same expansion in `Collector.collectEveryContainer`**, with every read — the pod and each per-container log — still resolved through `policy.Resolve`, so it adds no capability the policy package would not already have allowed. the agent's container order (init, regular, ephemeral, each in spec order) is deterministic, which kubectl's is **not** — `--all-containers` fetches concurrently and measured 22/7/1 init/sidecar/app-first over 30 live reads, so the original "kubectl's order, established against a live cluster" was one reading of the common case; a silent container contributes nothing and is not an error; and the first container's error becomes the read's error, which is what keeps `PodPreviousLogsCollector`'s "previous terminated" → EMPTY mapping identical on both paths. Five Go tests including a control proving the pod read happens *only* when the flag is set, three mutations all caught, and **verified by reverting the defect into a live harness**: with F24 present the sidecar pod reads `success: false` and `a container name must be specified ... choose one of: [app sidecar]`; with the fix, `APP-BOOT / FATAL-app-died / SIDECAR-PROXY-READY`, byte for byte what `kubectl logs --all-containers=true` returns. A scoped differential over the same pod gives 55 evidence records and zero status differences between providers. Found by the parity check written the same day for its neighbour, which asks whether a parameter the platform sends is one the agent reads at all — and which then failed on its own stale exception when the fix landed, which is what took `all_containers` off the ignored list. | done |
...
`src/lib/remediation.ts` and **gained their first tests** — and were later
deleted as F31, because every one of those tests verified the YAML and none the
choice of workload, which was wrong. Two dead components went with it (`MultiClusterPanel`,
