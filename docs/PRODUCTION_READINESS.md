...
| F21 | **M8a's routing and refusal are both inert on a worker that does not itself run a gateway.** The fleet presence index is installed inside the `agent_gateway_enabled` branch of `app/state.py`, so `get_agent_presence()` is `None` there: `agent_affinity` returns the shared queue and `select_provider` falls through to the local kubeconfig without ever consulting presence — no routing, and no refusal. The shipped topology cannot reach it (one Deployment, one config, N replicas), but a fleet mid-way through enabling `AGENT_GATEWAY_PORT` can, and there tenant A's `prod` is answered from a local context that merely shares the name — precisely what the refusal exists to prevent. Found by a soak whose first worker had a gateway and whose second did not: a third of investigations came back `provider=kubeconfig` with an agent attached and no refusal logged anywhere. The fix is to install presence under `distributed_state` rather than under the gateway, and to split `select_provider`'s grpc-requiring registry lookup from its Redis-only presence lookup. Not taken during the soak session, because it changes startup wiring on a load-bearing path. | 0.5d |
