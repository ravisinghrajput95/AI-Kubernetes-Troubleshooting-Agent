...
**The selection rationale names the reason that applied**, or nothing: the scope, a refuted more-confident cause, or a model's choice. It used to always cite severity — "ranked critical rather than critical" — which had been false since `rank()` moved confidence ahead of severity. **And it is rendered in the report's Root Cause section** — its only consumer was `HypothesisPanel`, which no route mounts, so it had been computed and shown nowhere since the audit that asked for it.
