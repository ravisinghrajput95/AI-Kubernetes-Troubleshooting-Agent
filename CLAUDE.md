...
**An expired lease proves a dead worker only if the worker could have renewed it.** With Postgres `docker pause`d past `JOB_LEASE_SECONDS`, no worker can write a renewal, every running lease expires, and the reaper's first tick after recovery failed a live investigation "Investigation worker stopped before the run finished" — which then completed on its still-running worker, leaving a record `succeeded` with that error and a console reading Failed for good. The reaper now reaps only after the store has answered it for a whole lease, and **a probe that hung and then answered counts as an outage too**: the first version reset only on raised errors, and on the worker whose query was already in flight when the store paused nothing raised — it waited ninety-five seconds, returned, and reaped on the same tick. `mark_succeeded` clears `error` — to `''`, because the column is `NOT NULL` and `NULL` failed every settle on real Postgres, which only running the contract suite against a database showed. Verified by pausing Postgres for 95s mid-investigation (no reap, succeeded, console truthful throughout) and, as the control, `kill -9` of a worker mid-run (reaped in 70s, the message now true). `tests/test_reaper_after_store_outage.py`.
...
**The gateway set no message size, so gRPC's 4 MiB default was the transport's real ceiling** — below the platform's own. `MAX_LIST_ITEMS` allows 2,000 objects and a pod list crosses 4 MiB at about 4,700, so a large cluster's list read was refused `RESOURCE_EXHAUSTED`, and **a refusal there ends the `Connect` stream**: the agent reconnects and everything in flight for that cluster goes with it, rather than one read degrading into a citable gap. Measured against a real gateway with an enrolled agent — 3.7 MB accepted, 11.2 MB refused, `Received message larger than max (11160087 vs. 4194304)`. `AGENT_MAX_MESSAGE_BYTES` (32 MiB) is it now, chosen against the platform's own ceiling (25,000 pods is 21.9 MB) and deliberately still a number: an agent is a customer's process and its payload is attacker-influenced, so unlimited is not the alternative. It bounds acceptance, not arrival — the payload is still one message, and chunked evidence is the wire change that would fix that.
...
**`CreateContainerConfigError` has its own playbook, and for a long time did not.** The reference resolver ran only under CrashLoop, and a pod waiting on a missing ConfigMap never starts, so it never loops. Every pipeline fixture was a crash-looping pod, where the resolver always happened to run — so the canonical missing-ConfigMap fault shipped a plan reading `ConfigMap <name-from-the-pod-spec>` while the pod's own events named the object. `ConfigurationPlaybook` closes it; `tests/test_configuration_investigation.py` drives the whole pipeline with a pod in that state.

**A failed read is not an absence, and "not found" is a flag, not a substring.** `ConfigReferenceCollector` records `exists` as `True` / `False` / `None`, and `ConfigReferenceRule` fires only on `False`. It used to treat every failed read as a missing object, and live — with the caller's RBAC lifted from the shipped read-only grant, which correctly excludes Secrets — a Secret that **existed** became the root cause of every investigation of its namespace on **both** providers: Forbidden through a kubeconfig, unsupported (`describe`) through an agent. Absence itself travels as `ProviderResult.not_found`: kubectl's `(NotFound)` on the kubeconfig path, a named `EMPTY` on the agent path — the agent emits `EMPTY` for nothing else. Matching kubectl's stderr text meant the agent path could never name a missing object. A not-found result is never cached. `tests/test_not_found_parity.py` puts both providers' real shapes through the same resolver.

`playbook_rounds[].hypotheses_before` records what the analysis concluded before the round. The report's "the initial pass alone would not have reached this conclusion" was unconditional, and false whenever the baseline had already selected the same cause.
...
**A hypothesis names one resource, so it is scored on that resource's evidence.** A rule fires on every resource its triggers match, and scoping support to "the triggered workloads" still pooled them: after a node restart the in-cluster agent pod — Ready for nine minutes, one exit code — was the root cause at 92% on checkout's FATAL log and metrics-server's probe, and the tie rationale quoted the pool ("21 against 9"). `SignalPatternRule` now scores each workload apart (a Deployment and its `<name>-<hash>-<id>` pods, a StatefulSet and its ordinals, are one) and the hypothesis is the best-evidenced workload, preferring one nothing refutes; replicas of one workload still corroborate each other, unrelated workloads do not. Two further scopes came out of the same capture: a hypothesis about a namespaced object is never supported from another namespace, and one about a Service only by the Service itself and the pods its trigger says it selects. And the graph rules no longer count a pod listed only for restart history (`reported_now: false`) as failing — metrics-server's Service read "no healthy backend" on a Ready pod. The kubeconfig and agent reads of that cluster, seconds apart, named **different** root causes before (one pod's restart aged past `STABLE_AFTER` between them and moved a pooled confidence) and rank identically after; `tests/fixtures/real_investigations_after_restart_two_providers.json` is both captures.
...
**The selection rationale names the reason that applied**, or nothing: the scope, a refuted more-confident cause, or a model's choice. It used to always cite severity — "ranked critical rather than critical" — which had been false since `rank()` moved confidence ahead of severity. **And it is rendered in the report's Root Cause section** — its only consumer was `HypothesisPanel`, which no route mounts, so it had been computed and shown nowhere since the audit that asked for it. **When nothing but breadth ordered two causes, it says that instead of staying silent.** Two faults on different pods, both 80% and critical, were listed as "root cause" and "alternative" with no reason given, and the only thing that ordered them was which rested on more signals. `_tie` names both and says to treat them as concurrent faults, not a cause and its alternatives.
...
- **An uncollected owner is not an absent one.** `workload_ref()` falls back to the pod whenever no pod spec was collected, and every helper read that fallback as an observed bare pod — "No controller owns pod/notifier-…" about a ReplicaSet-owned pod. `context.ownership_known()` is false for a Pod with no spec, and `_rollout_undo`, `_restart_step` and `_unmanaged_caveat` then say ownership was not collected instead.
...
**Guidance is keyed on the selected hypothesis, never on payload order (F31 in prose).** `FixRecommendationEngine` used to substring-match `str(investigation)` and aim every command and follow-up at the first unhealthy deployment. In a namespace with nine faults that produced, for a missing-ConfigMap diagnosis, image-tag prevention advice and `kubectl edit deployment checkout` — which MCP handed to an agent verbatim. Commands are now the remediation plan's, prevention is keyed on `Hypothesis.category`, and the follow-up names the plan's target. On the grounded path the plan is built for the hypothesis the **model** selected, not the deterministic leader. Every earlier fixture had one fault, where "first" and "diagnosed" are the same object; `tests/test_fix_recommendation.py` uses the captured nine-fault pod list with an unrelated deployment first.
...
- **A label says who wrote the text, and was keyed on the section title.** Resolution, Lessons Learned and Preventive Actions carried "◆ Model-authored · not evidence-derived" on every report, including every report from a deployment with no model configured, under "Diagnosis source: Deterministic analysis" — and Resolution's body is the remediation plan, the most evidence-derived section there is. `sectionProvenance` asks whether a model answered; deterministic prevention is labelled general guidance for the fault.
- **A trend is a share of runs, not a spread of timestamps.** `trendOf` compared when a finding's occurrences fell and ignored runs where it was absent, so three quick runs and one later one labelled every always-present finding "happening less often". It takes the runs of the finding's clusters and refuses when either half of the window holds fewer than two.
- **An unchecked check is not a warning.** "High CVEs Found" was a security *warning* on a deployment with no scanner, and "No Privileged Containers" warned about a privileged container: labels named outcomes whatever the status, and the list was `status !== "pass"` — an earlier fix had unified count and list on the wrong predicate. Checks now carry an `id` (what `ResourceLimitsRule` matches) and a neutral label; `unknown` renders under "Not checked". Coverage "Gaps" excludes `not_applicable`, which read "Completeness 100%" beside "Gaps 11".
- **A view of the cluster speaks from a whole-cluster run.** The fleet card and the cluster page took the newest run, so an investigation of one deployment became the cluster's headline and its seven pods the cluster's capacity. History items now record `scope` (on the live save *and* the regenerate path — the first version wrote it only on regenerate, where the console's fix read nothing), and `representativeRun` prefers the newest whole-cluster run; when only scoped runs exist the card says what was read. Entries written before `scope` count as whole-cluster, which is how they were always treated.
- **The platform not answering is not the investigation failing.** `attach` set the phase to `failed` on any error, so one 503 during a database pause marked a running investigation Failed, permanently — nothing retried. A 404 or 403 is still an answer; a network error, a timeout or a 5xx now says the platform could not be reached and retries. The header reads `/health/ready` too: "Connected" had meant only that `/health` answered, and it said so for the whole pause while both workers were not ready. It now says Unavailable or Degraded, naming the dependency.
- **A page about a cluster sets the scope.** The header's switcher read only `?cluster=` and adopted the kubeconfig default, so `/clusters/sweep-agent` and a `sweep-agent` investigation both showed `kind-k8s-agent-dev` in the header.
...
- **A state nothing tracks.** Report Status read Open or Resolved. Environment was a substring guess (`kind-prod` → Development). "Secrets were scrubbed from this record" was on every record that went through the redactor. Lessons Learned offered "Prometheus is not configured" as what would have shortened diagnosing a selector mismatch. "Business Impact" was three hedges picked by which sections had findings ("Affected workloads may be unavailable or unstable") on every report of a nine-fault namespace; it is "What is affected" now, each line naming what was observed, a failed read or a truncated list said rather than passed over, and restart history kept apart from failing now.
...
```bash
python scripts/mutation_check.py                   # 154 mutations
...
| `docs/SLO.md` | Proposed targets; **measured once, 19 minutes on one host** (six met, soundness unmeasured — the Mac idle-slept; run soaks under `caffeinate -i`), not production attainment; soundness is two-sided and queue depth means "add workers, not slots" |
