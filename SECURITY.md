...
| Dependency audit | All three stacks, and each fails the build: `pip-audit --strict` for the platform, `govulncheck` (source mode, so a finding means a call path) for the cluster agent, `npm audit` at any severity for the console. The Go toolchain is pinned in `agent/go.mod`. Until 2026-09 only the Python stack was audited, and v0.3.0's agent image shipped gRPC 1.83.0 and `x/text` 0.37.0 with reachable advisories. |
| Webhook signatures | HMAC-SHA256 over the timestamp and the body, compared in constant time, five-minute window; a non-finite timestamp is refused (`nan` used to pass the window forever). |
| Identifier validation | Cluster, tenant and investigation ids are matched whole (`fullmatch`): `re.match` with `^…$` accepted a trailing newline, a second identity printing like the first. |
| Console container | Static files served by unprivileged nginx. It used to run Vite's development server as root, published by compose on every interface. |
...
- **Peak parse memory is proportional to cluster size on the agent path.**
  Through a kubeconfig a list read is decoded as it arrives, and peak memory is
  flat at 8.4 MB from 5,000 to 25,000 pods (F5). Through an agent,
  `decode_payload` runs `json.loads` over the whole payload before the item cap
  applies, so the cap bounds the stored result and not the spike. It was 5.9 MB
  at 2,000 pods, 29.7 MB at 10,000 and 74.3 MB at 25,000 on the kubeconfig path
  before streaming; the agent path has that shape by construction and has not
  been measured.
